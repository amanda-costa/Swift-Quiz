...
class QuizManeger{
    ...
    private var quiz: Quiz! // é privado por que eu nao quero que o app tenha acesso ao quiz, a necessidade é que ele apenas receba as perguntas e respostas
    private var _totalAnswers = 0
    private var _totalCorrectAnswers = 0
    
    // fazer com o que a classe quiz tenha acesso apenas as perguntas e respostas, no caso irei criar uma variavel computada
    ...
    // essa funcao será responsavel por pegar um quiz aleatorio
    func refreshQuiz() {
        let randomIndex = Int(arc4random_uniform(UInt32(quizes.count)))// vai gerar um indice aleatorio
